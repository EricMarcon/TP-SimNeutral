...
evolve_drift <- function(community, show = FALSE) {
  ...
  if (show) {
    ...
    print(
      autoplot(community_to_plot) + labs(colour = "Species", size = "Size")
    )
  }
  # return
  return(community)
...
evolve_migrate <- function(community, metacommunity, m = 0) {
  ...
  if (u > m) {
    ...
    sp_names <- colnames(abd_species(metacommunity))
    # Metacommunity species abundances
    sp_abd <- as.numeric(metacommunity)
    # Draw a species in the metacommunity:
    # a vector of species abd with a single value 1
    mc_1_tree <- rmultinom(1, size = 1, prob = sp_abd)
    # Set the new species
    marks(community)$PointType[focus_tree] <- sp_names[mc_1_tree == 1]
  }
  # return
  return(community)
}
...
```{r}
# Hint
rcommunity(1, size = 256, species_number = 10) %>% 
  autoplot()
```
