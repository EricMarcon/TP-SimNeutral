...
Its estimated number of species is `r n_species`.
...
Simulating the abundance of the `r n_species` of Amazonia takes a few hours.
...
c_abundances[c_abundances > 0] %>% 
  # Declare an vector of abundances
  as.AbdVector() ->
  c_abundances
# Show the RAC
...
c_wmppp$marks$PointType <- factor(c_wmppp$marks$PointType, levels = names(mc_abundances))
...
evolve_drift <- function(community, show = FALSE) {
  ...
  community$marks$PointType[focus_tree] <- 
    survivors_sp[sample(length(survivors_sp), size = 1)]
  if (show) {
    # Copy the community
